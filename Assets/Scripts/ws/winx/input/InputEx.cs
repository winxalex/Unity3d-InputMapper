...
//		static 		int _lastFrame=-1;
		...
		public static bool anyKeyDown {
			get{ return Input.anyKeyDown || anyKeyDownOnAny();
			}
		}
		...
		public static float GetAxis (InputAction action)
		{
			...
			if(action.fromAny){//first device that have value not equal 0 or return 0
				...
				return InputManager.hidInterface.Devices[KeyCodeExtension.toJoystickID(code)].GetAxis(code);
			}
		}
		...
		public static bool GetKey(int code,bool fromAny=false){
			...
			if(code<KeyCodeExtension.MAX_KEY_CODE){
				...
				if(fromAny){
					...
					return InputManager.hidInterface.Devices[KeyCodeExtension.toJoystickID(code)].GetKey(code);
				}
			}
		}
			...
			if(code<KeyCodeExtension.MAX_KEY_CODE){
					return Input.GetKeyUp((KeyCode)code);
						...
						if(action.fromAny){
							...
							return InputManager.hidInterface.Devices[KeyCodeExtension.toJoystickID(code)].GetKeyUp(code);
						}
		...
		public static bool GetKeyDown(InputAction action){
			...
			if(code<KeyCodeExtension.MAX_KEY_CODE){
				...
				if(action.fromAny){
					...
					return InputManager.hidInterface.Devices[KeyCodeExtension.toJoystickID(code)].GetKeyDown(code);
				}	
			}
		}
