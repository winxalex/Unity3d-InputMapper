...
namespace ws.winx.platform.osx
{
	...
	public class GenericHIDDevice:HIDDevice
	{
		...
		readonly IntPtr RunLoop = Native.CFRunLoopGetCurrent();//Native.CFRunLoopGetCurrent ();
...
//					_scheduleWithRunLoop=true;
	...
	//				_registeredToInputReport=true;
		...
		public override void Dispose ()
		{
			...
			Debug.Log ("Try to free InputBuffer Global");
			if (__InputBufferPtr != IntPtr.Zero)
								Marshal.FreeHGlobal (__InputBufferPtr);

			Debug.Log ("Try to free OutputBuffer Global");
			if (__OutputBufferPtr != IntPtr.Zero)
				Marshal.FreeHGlobal (__OutputBufferPtr);

			Debug.Log ("Try to free GCHandle");
			if (DeviceGCHandle.IsAllocated) {
				DeviceGCHandle.Free ();
			}


			//Fucken OSX is shit. No documentation no internet resources including stackoverflow, new errors with every sub sub version.
			//no try catch with native code and 
			if (__deviceHandle != IntPtr.Zero) {


				//produce crash
//				Debug.Log ("Try to unrschedule device from RunLoop");
//				try{
//				if(__deviceHandle!=IntPtr.Zero && RunLoop!=IntPtr.Zero && _scheduleWithRunLoop)
...
//				}
				...
				//click fast play stop in iDE this would blow too
				//Debug.Log ("Try to unregister from InputReport");
								//Native.IOHIDReportCallback empty = null;
								//			Native.IOHIDDeviceRegisterInputReportCallback (__deviceHandle, __InputBufferPtr, InputReportByteLength, empty,
								//			                                               
								//			                                               GCHandle.ToIntPtr (DeviceGCHandle));
							//	if(_registeredToInputReport)
							//	Native.IOHIDDeviceRegisterInputReportCallback (__deviceHandle, IntPtr.Zero, 0, empty, IntPtr.Zero);


								
								
								Debug.Log ("Try to close device");

				//produce craches
								//CloseDevice ();

				//Native.CFRelease(__deviceHandle);
			}
